% plot parameters
lw = 1.5;                   % linewidth
opac = 0.2;                 % opacity of shading
tlim = [250 300];           % plot bounds on time axis (for zoom)
flim = [0 2];               % plot bounds on freq. axis 
...
ax1a = nexttile; 
    ...
    patch([time'; flipud(time')],[ytop'; flipud(ybot')], 'r', 'FaceAlpha', opac, 'EdgeColor', 'none');  
...
ax3a = nexttile; 
    ...
    % measurements
    if ismember(d, Sa_dof)
        plot(t_n, acc_n(find(Sa_dof==d),:), 'o', 'Color', orange);
    end

    % labels
    xlabel('t');
    ylabel('a(t)');
    legend(["true", "est. \mu", "est. \mu \pm 1 \sigma", "measurements"], 'Location', 'northwest');
...
for i = 1:nmodes
    ...
    ax1 = nexttile;
        ...
        xlim(tlim);
        if i == 3
            legend("true", "est. \mu", "est. \mu \pm 1 \sigma");
            xlabel('t');
        end
...
zlabel("$log(d(p, \hat{p}))$", 'interpreter', 'latex');
